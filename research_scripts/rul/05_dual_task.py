...
#
# Method group **G16**: Dual-task OP and gradient model. Architecture: CNN-Transformer encoder. Method tags: dual task|SOH auxiliary|cycle gradient.
#
...
# %%
plt.plot(bat_dict["b2c12"]["summary"]["QD"])
...
def data_maker(inter_list, window_size):
    ...
    for battery_key in inter_list:
        ...
        for i in range(len(train_df) - window_size + 1):
            ...
            train_y_RUL.append(np.array(train_df.iloc[i + window_size - 1, 2], dtype=np.float32))
            train_y_SOH.append(np.array(train_df.iloc[i + window_size - 1, 3], dtype=np.float32))
            train_x_Cycle.append(np.array(train_df.iloc[i + window_size - 1, 1], dtype=np.float32))
    ...
    train_y_RUL = np.array(train_y_RUL)
    train_y_SOH = np.array(train_y_SOH)
    train_x_OP = np.array(train_x_OP)
    train_x_Cycle = np.array(train_x_Cycle)
    ...
    train_x_Cycle = np.reshape(train_x_Cycle, (train_x_Cycle.shape[0], 1))

    return train_x, train_x_OP, train_x_Cycle, train_y_SOH, train_y_RUL


# %%
train_x, train_x_OP, train_x_Cycle, train_y_SOH, train_y_RUL = data_maker(train_bat_key, 50)

# %%
val_x, val_x_OP, val_x_Cycle, val_y_SOH, val_y_RUL = data_maker(val_bat_key, 50)
...
val_x_Cycle_tensor = torch.tensor(val_x_Cycle, dtype=torch.float32).to(DEVICE)
val_y_SOH_tensor = torch.tensor(val_y_SOH, dtype=torch.float32).to(DEVICE)
val_y_RUL_tensor = torch.tensor(val_y_RUL, dtype=torch.float32).to(DEVICE)
...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.X1[idx], self.X2[idx], self.X3[idx], self.y1[idx], self.y2[idx]


dataset = MyDataset(
    torch.tensor(train_x, dtype=torch.float32),
    torch.tensor(train_x_OP, dtype=torch.float32),
    torch.tensor(train_x_Cycle, dtype=torch.float32, requires_grad=True),
    torch.tensor(train_y_SOH, dtype=torch.float32),
    torch.tensor(train_y_RUL, dtype=torch.float32),
)
...
def objective(trial):
    ...
    for epoch in range(0, 200):
        ...
        for batch_X1, batch_X2, batch_y1 in data_loader:
            ...
            with torch.no_grad():
                model.eval()
                SOH_pred_val = model(val_x_tensor, val_x_OP_tensor)
...
# %%
model = SOH_model(6, 50, 4, 4).to(DEVICE)
model = model.train()

lam_1 = torch.tensor([1.0]).to(DEVICE)
lam_2 = torch.tensor([2.0]).to(DEVICE)
...
loss_fn_physics = torch.nn.L1Loss()
...
for epoch in range(0, 300):
    ...
    RUL_loss = []
    physics_loss = []
    for batch_X1, batch_X2, batch_X3, batch_y1, batch_y2 in data_loader:
        model.train()
        batch_X1 = batch_X1.to(DEVICE)
        batch_X2 = batch_X2.to(DEVICE)
        ...
        SOH_pred, RUL_pred = model(batch_X1, batch_X2, batch_X3)

        RUL_N = gradient_calculator(RUL_pred, batch_X3).squeeze(1)

        l1 = torch.sqrt(loss_fn1(SOH_pred, batch_y1))

        l2 = torch.sqrt(loss_fn2(RUL_pred, batch_y2))

        physics_value = RUL_N + 1

        empty_tensor = torch.zeros(len(physics_value)).to(DEVICE)

        l3 = loss_fn_physics(physics_value, empty_tensor)

        loss = lam_1 * l1 + lam_2 * l2 + lam_3 * l3 - torch.log(lam_1 * lam_2 * lam_3)
        # loss = l1 + l2
        # loss = l1

        SOH_loss.append(float(l1))
        RUL_loss.append(float(l2))
        physics_loss.append(float(l3))
        ...
        with torch.no_grad():
            model.eval()
            SOH_pred_val, RUL_pred_val = model(val_x_tensor, val_x_OP_tensor, val_x_Cycle_tensor)
            SOH_pred_val = SOH_pred_val.cpu().detach().numpy()
            RUL_pred_val = RUL_pred_val.cpu().detach().numpy()
            RUL_val_loss = mean_squared_error(val_y_RUL, RUL_pred_val) ** 0.5
            # SOH_val_loss = mape_loss(val_y_tensor, SOH_pred_val).cpu().detach().numpy()
            RUL_val_loss = float(RUL_val_loss)

        if RUL_val_loss < SOH_loss_best:
            SOH_loss_best = RUL_val_loss
            torch.save(model.state_dict(), artifact_path("RUL_prediction_OP_50_gradient_dual_task"))
        # print(float(l1), float(l2), SOH_val_loss)

    SOH_loss = np.array(SOH_loss).mean()
    RUL_loss = np.array(RUL_loss).mean()
    physics_loss = np.array(physics_loss).mean()
    print(
        "epoch : ",
        epoch,
        "/SOH_loss :",
        round(float(SOH_loss), 4),
        "/RUL_loss :",
        round(float(RUL_loss), 4),
        "/physics_loss :",
...
for k in range(0, 5):
    ...
    model_new = SOH_model(6, 50, 4, 4).to(DEVICE)

    model_new.register_parameter("lam_1", lam_1)
    model_new.register_parameter("lam_2", lam_2)
    model_new.register_parameter("lam_3", lam_3)

    model_new.load_state_dict(torch.load(artifact_path("RUL_prediction_OP_50_gradient_dual_task")))

    test_x, test_x_OP, test_x_Cycle, test_y_SOH, test_y_RUL = data_maker(battery_list, 50)

    test_x_tensor = torch.tensor(test_x, dtype=torch.float32).to(DEVICE)
    test_x_OP_tensor = torch.tensor(test_x_OP, dtype=torch.float32).to(DEVICE)
    test_x_Cycle_tensor = torch.tensor(test_x_Cycle, dtype=torch.float32).to(DEVICE)
    test_y_SOH_tensor = torch.tensor(test_y_SOH, dtype=torch.float32).to(DEVICE)
    test_y_RUL_tensor = torch.tensor(test_y_RUL, dtype=torch.float32).to(DEVICE)

    test_y_RUL = test_y_RUL * 1500

    with torch.no_grad():
        model_new.eval()
        SOH_test_pred, RUL_test_pred = model_new(
            test_x_tensor, test_x_OP_tensor, test_x_Cycle_tensor
        )
        SOH_test_pred = SOH_test_pred.cpu().detach().numpy()
        RUL_test_pred = RUL_test_pred.cpu().detach().numpy() * 1500
    ...
    if mode == "RUL":
        soh_rmse = mean_squared_error(test_y_RUL, RUL_test_pred) ** 0.5
        soh_mape = (
            mean_absolute_error(np.array(test_y_RUL), np.array(RUL_test_pred)) / test_y_RUL[0]
        ) * 100
        EOL_mape = MAPE(np.array(test_y_RUL[0]), np.array(RUL_test_pred[0]))

        rmse_list.append(soh_rmse)
        mape_list.append(soh_mape)
        data_length = len(np.array(test_y_RUL))

        plt.subplot(plot_num)
        plt.plot(RUL_test_pred, color="r", label="Predicted RUL")
        plt.plot(test_y_RUL, color="b", label="real RUL")
