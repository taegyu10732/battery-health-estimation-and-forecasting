...
#
# Method group **G20**: Weighted multi-cycle fusion. Architecture: CNN-Transformer encoder. Method tags: multi-cycle|learned weighting.
#
...
class RUL_model(nn.Module):
    def __init__(self, in_features, time_length, first_head, first_layer):
        ...
        self.linear_1_final = nn.Linear(600, 1)
        ...
        self.linear_2_final = nn.Linear(600, 1)
        ...
        self.linear_3_final = nn.Linear(600, 1)
    ...
    def forward(self, x_1, x_2, x_3):
        ...
        x_1_weight = x_1.reshape(-1, 1, 600)
        x_1_weight = self.linear_1_final(x_1_weight)
        x_1_weight = self.sigmoid_1(x_1_weight)
        ...
        x_2_weight = x_2.reshape(-1, 1, 600)
        x_2_weight = self.linear_2_final(x_2_weight)
        x_2_weight = self.sigmoid_2(x_2_weight)
        # ---------------------------------------------------------------------------------
        ...
        x_3_weight = x_3.reshape(-1, 1, 600)
        x_3_weight = self.linear_2_final(x_3_weight)
        x_3_weight = self.sigmoid_3(x_3_weight)
        # --------------------------------------------------------------------------------

        x = x_1_weight * x_1 + x_2_weight * x_2 + x_3_weight * x_3
        ...
        return (
            x.squeeze(1),
            x_1_weight.squeeze(1).squeeze(1),
            x_2_weight.squeeze(1).squeeze(1),
            x_3_weight.squeeze(1).squeeze(1),
        )
...
for epoch in range(0, 300):
    ...
    for batch_X1, batch_X2, batch_X3, batch_y1 in data_loader:
        ...
        batch_X3 = batch_X3.to(DEVICE)
        batch_y1 = batch_y1.to(DEVICE)
        SOH_pred, _, _, _ = model(batch_X1, batch_X2, batch_X3)
        loss = torch.sqrt(loss_fn(SOH_pred, batch_y1))
        # loss = mape_loss(SOH_pred, batch_y1)
        # loss =loss_fn(SOH_pred, batch_y1)
        ...
        with torch.no_grad():
            model.eval()
            SOH_pred_val, _, _, _ = model(val_x_long_tensor, val_x_mid_tensor, val_x_short_tensor)
            SOH_pred_val = SOH_pred_val.cpu().detach().numpy()
            SOH_val_loss = mean_squared_error(val_y, SOH_pred_val) ** 0.5
            # SOH_val_loss = mape_loss(val_y_tensor, SOH_pred_val).cpu().detach().numpy()
            SOH_val_loss = float(SOH_val_loss)
        ...
        if SOH_val_loss < SOH_loss_best:
            ...
            torch.save(
                model.state_dict(),
                artifact_path("RUL_prediction_multiple_cycle_input_weighted_plus_1"),
            )
            print(
                "epoch : ",
                epoch,
...
for k in range(len(test_bat_key) - 6):
    ...
    model_new.load_state_dict(
        torch.load(artifact_path("RUL_prediction_multiple_cycle_input_weighted_plus_1"))
    )
    ...
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred, _, _, _ = model_new(
            test_x_long_tensor, test_x_mid_tensor, test_x_short_tensor
        )
...
for k in range(0, 1):
    ...
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred, w1, w2, w3 = model_new(
            test_x_long_tensor, test_x_mid_tensor, test_x_short_tensor
        )
        # SOH_test_pred = model_new(test_x_long_tensor,test_x_mid_tensor,test_x_short_tensor)
        SOH_test_pred = SOH_test_pred.cpu().detach().numpy() * 1500
...
# %%
plt.plot(w1.cpu(), label="w30")
plt.plot(w2.cpu(), label="w20")
plt.plot(w3.cpu(), label="w10")
