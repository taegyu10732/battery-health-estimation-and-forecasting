...
#
# Method group **G14**: CNN-LSTM RUL model. Architecture: CNN-LSTM. Method tags: sequence window|operating profile.
#
...
for battery_key in selected_keys:
    bat_key.append(battery_key)
    protocol = extract_numbers(bat_dict[battery_key]["charge_policy"])
    pro_1.append(protocol[0])
    pro_2.append(protocol[1] / 100)
    pro_3.append(protocol[2])
    pro_4.append(1 - ((protocol[1]) / 100))
    for k in range(0, len(bat_dict[battery_key]["summary"]["QD"])):
        ...
        if temp_SOH < 80.3:
            ...
            EOL.append(EOL_Cycle)
            break

# %%
df = pd.DataFrame(
    {"bat_key": bat_key, "pro_1": pro_1, "pro_2": pro_2, "pro_3": pro_3, "pro_4": pro_4, "EOL": EOL}
)

# %%
plt.scatter((df["pro_1"] ** 4) * (df["pro_2"]) * (df["pro_3"] ** 2) * (df["pro_4"]), df["EOL"])

# %%
plt.scatter(df["pro_1"], df["EOL"])
...
class SOH_model(nn.Module):
    def __init__(self, in_features, time_length, first_head, first_layer):
        ...
        self.lstm = nn.LSTM(6, 400, num_layers=2, batch_first=True)
        self.lstm_linear = nn.Linear(400, 6)
...
for epoch in range(0, 300):
    ...
    for (
        ...
        if SOH_val_loss < SOH_loss_best:
            SOH_loss_best = SOH_val_loss
            torch.save(model.state_dict(), artifact_path("RUL_prediction_OP_lstm"))
...
for k in range(0, 9):
    ...
    model_new.load_state_dict(torch.load(artifact_path("RUL_prediction_OP_lstm")))
    ...
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred = model_new(test_x_tensor, test_x_OP_tensor)
