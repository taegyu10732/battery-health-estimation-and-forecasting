...
#
# Method group **G19**: Multi-cycle SOH fusion. Architecture: CNN-Transformer encoder. Method tags: multi-cycle|SOH auxiliary.
#
...
for bat_key in train_bat_key:
    ...
    for i in range(len(cell_df) - 29):
        ...
        train_x_SOH.append(np.array(cell_df.iloc[i : i + 30, 3], dtype=np.float32))
...
train_x_SOH = np.array(train_x_SOH)
...
for bat_key in val_bat_key:
    ...
    for i in range(len(cell_df) - 29):
        ...
        val_x_SOH.append(np.array(cell_df.iloc[i : i + 30, 3], dtype=np.float32))
...
train_x_SOH_tensor = torch.tensor(train_x_SOH, dtype=torch.float32)
...
val_x_SOH_tensor = torch.tensor(val_x_SOH, dtype=torch.float32).to(DEVICE)
val_y_tensor = torch.tensor(val_y, dtype=torch.float32).to(DEVICE)


# %%
class MyDataset(Dataset):
    def __init__(self, X1, X2, X3, X4, y1):
        self.X1 = X1
        self.X2 = X2
        self.X3 = X3
        self.X4 = X4
    ...
    def __getitem__(self, idx):
        return self.X1[idx], self.X2[idx], self.X3[idx], self.X4[idx], self.y1[idx]


dataset = MyDataset(
    train_x_long_tensor,
    train_x_mid_tensor,
    train_x_short_tensor,
    train_x_SOH_tensor,
    train_y_tensor,
)
...
class RUL_model(nn.Module):
    def __init__(self, in_features, time_length, first_head, first_layer):
        ...
        self.trunk1 = nn.Linear(30, 300)
        self.trunk2 = nn.Linear(300, 300)
        self.trunk3 = nn.Linear(300, 300)
        self.trunk4 = nn.Linear(300, 300)
        ...
        self.linear_1_final = nn.Linear(600, 300)
    ...
    def forward(self, x_1, x_2, x_3, x_4):
        ...
        x = self.encoder(x)
        x = x.reshape(-1, 600)
        x = self.linear_1_final(x)

        # ---------------------------------------------------------------------------------

        Final = torch.mul(x, x_4)
...
for epoch in range(0, 300):
    idx = 0
    for batch_X1, batch_X2, batch_X3, batch_X4, batch_y1 in data_loader:
        model.train()
        batch_X1 = batch_X1.to(DEVICE)
        batch_X2 = batch_X2.to(DEVICE)
        batch_X3 = batch_X3.to(DEVICE)
        batch_X4 = batch_X4.to(DEVICE)
        batch_y1 = batch_y1.to(DEVICE)
        SOH_pred = model(batch_X1, batch_X2, batch_X3, batch_X4)
        loss = torch.sqrt(loss_fn(SOH_pred, batch_y1))
        # loss = mape_loss(SOH_pred, batch_y1)
        # loss =loss_fn(SOH_pred, batch_y1)
        ...
        with torch.no_grad():
            model.eval()
            SOH_pred_val = model(
                val_x_long_tensor, val_x_mid_tensor, val_x_short_tensor, val_x_SOH_tensor
            )
            SOH_pred_val = SOH_pred_val.cpu().detach().numpy()
            SOH_val_loss = mean_squared_error(val_y, SOH_pred_val) ** 0.5
            # SOH_val_loss = mape_loss(val_y_tensor, SOH_pred_val).cpu().detach().numpy()
        ...
        if SOH_val_loss < SOH_loss_best:
            ...
            torch.save(
                model.state_dict(),
                artifact_path("RUL_prediction_multiple_cycle_input_plus_SOH_added"),
            )
            print(
                "epoch : ",
                epoch,
...
for k in range(len(test_bat_key) - 6):
    ...
    model_new.load_state_dict(
        torch.load(artifact_path("RUL_prediction_multiple_cycle_input_plus_SOH_added"))
    )
    ...
    for i in range(len(test_df) - 29):
        ...
        test_x_SOH.append(np.array(test_df.iloc[i : i + 30, 3], dtype=np.float32))
    ...
    test_x_SOH = np.array(test_x_SOH)
    ...
    test_x_SOH_tensor = torch.tensor(test_x_SOH, dtype=torch.float32).to(DEVICE)
    test_y_tensor = torch.tensor(test_y, dtype=torch.float32).to(DEVICE)

    # print(test_x.shape)
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred = model_new(
            test_x_long_tensor, test_x_mid_tensor, test_x_short_tensor, test_x_SOH_tensor
        )
