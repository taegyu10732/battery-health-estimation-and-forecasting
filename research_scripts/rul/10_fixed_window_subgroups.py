...
#
# Method group **G21**: Fixed-window subgroup model. Architecture: CNN-Transformer encoder. Method tags: 30-cycle window|life subgroup|SOH regularization.
#
...
def data_maker(inter_list, window_size):
    ...
    for battery_key in inter_list:
        ...
        train_df = pd.DataFrame(
            {
                ...
                "SOH": (np.array(SOH_true) - 0.87) / (1.1 - 0.87),
        ...
        for i in range(len(train_df) - window_size + 1):
            ...
            train_x_SOH.append(np.array(train_df.iloc[i : i + window_size, 3], dtype=np.float32))
            train_y.append(np.array(train_df.iloc[i + window_size - 1, 2], dtype=np.float32))
            train_y_before.append(np.array(train_df.iloc[i + window_size - 2, 2], dtype=np.float32))
    ...
    train_y_before = np.array(train_y_before)

    train_x = np.reshape(train_x, (train_x.shape[0], 1, train_x.shape[1], train_x.shape[2]))
    train_x_SOH = np.reshape(train_x_SOH, (train_x_SOH.shape[0], window_size))

    return train_x, train_x_SOH, train_y, train_y_before, cleared_df, origin_df


# %%
train_x, train_x_SOH, train_y, train_y_before, cleared_df, origin_df = data_maker(train_bat_key, 30)

# %%
val_x, val_x_SOH, val_y, _, _, _ = data_maker(val_bat_key, 30)
...
dataset = MyDataset(
    ...
    torch.tensor(train_x_SOH, dtype=torch.float32),
    torch.tensor(train_y, dtype=torch.float32),
    torch.tensor(train_y_before, dtype=torch.float32),
)
...
class SOH_model(nn.Module):
    def __init__(self, in_features, time_length, first_head, first_layer):
        ...
        # Trunk Net ---------------------------
        self.Linear_T1 = nn.Linear(30, 100)
...
# %%
model = SOH_model(6, 30, 4, 4).to(DEVICE)
model = model.train()
...
for epoch in range(0, 300):
    ...
    for batch_X1, batch_X2, batch_y1, batch_y2 in data_loader:
        ...
        SOH_pred = torch.where(SOH_pred > batch_y2, SOH_pred * 1.5, SOH_pred)
        ...
        if SOH_val_loss < SOH_loss_best:
            SOH_loss_best = SOH_val_loss
            torch.save(
                model.state_dict(), artifact_path("RUL_prediction_with_SOH_filtered_regularization")
            )
...
for k in range(0, 9):
    ...
    battery_list = [train_bat_key[k]]

    model_new = SOH_model(6, 30, 4, 4).to(DEVICE)

    model_new.load_state_dict(
        torch.load(artifact_path("RUL_prediction_with_SOH_filtered_regularization"))
    )

    test_x, test_x_SOH, test_y, _, _, _ = data_maker(battery_list, 30)
    ...
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred = model_new(test_x_tensor, test_x_SOH_tensor)
