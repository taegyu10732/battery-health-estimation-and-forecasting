...
#
# Method group **G13**: Normalized RUL Transformer. Architecture: CNN-Transformer encoder. Method tags: Optuna|normalized inputs.
#
    ...
    for cycle_num in range(1, len(bat_dict[battery_key]["summary"]["cycle"])):
        ...
        if cycle_num != "1":
            ...
            if np.array(temporal_data).shape == (3, inter_value):
                ...
                SOH_true.append(EOL_Cycle - int(cycle_num))
...
SOH_true = np.array(SOH_true) / 1500
...
dataset = MyDataset(
    torch.tensor(whole_data, dtype=torch.float32), torch.tensor(SOH_true, dtype=torch.float32)
)
    ...
    for cycle_num in range(2, len(bat_dict[battery_key]["summary"]["cycle"])):
        ...
        if cycle_num != "1":
            ...
            if np.array(temporal_data).shape == (3, inter_value):
                ...
                SOH_val.append(EOL_Cycle - int(cycle_num))
...
# %%
model = SOH_model(3, inter_value, 10, 3).to(DEVICE)
model = model.train()
...
for epoch in range(0, 500):
    ...
    for batch_X1, batch_y1 in data_loader:
        ...
        SOH_pred_val = SOH_pred_val.cpu().detach().numpy()
        SOH_val_loss = mean_squared_error(SOH_val, SOH_pred_val) ** 0.5
        # SOH_val_loss = mape_loss(torch.tensor(SOH_val), SOH_pred_val)
        ...
        if SOH_val_loss < SOH_loss_best:
            SOH_loss_best = SOH_val_loss
            torch.save(model.state_dict(), artifact_path("New_normalization_RUL_prediction_1"))
            print(
                "epoch : ",
                epoch,
                "/idx : ",
...
for k in range(len(test_bat_key) - 6):
    ...
    model_new = SOH_model(3, inter_value, 10, 3).to(DEVICE)

    model_new.load_state_dict(torch.load(artifact_path("New_normalization_RUL_prediction_1")))
    model_new.eval()
        ...
        for cycle_num in range(1, len(bat_dict[battery_key]["summary"]["cycle"])):
            ...
            if cycle_num != "1":
                ...
                if np.array(temporal_data).shape == (3, inter_value):
                    ...
                    SOH_test.append(EOL_Cycle - int(cycle_num))
    ...
    SOH_test_pred = model_new(whole_test_data)
    SOH_test_pred = SOH_test_pred.cpu().detach().numpy() * 1500
    SOH_test = np.array(SOH_test)

    if battery_key == "b2c44":
        SOH_test_pred = np.delete(SOH_test_pred, [245])
        Cycle_test = np.delete(Cycle_test, [245])
        SOH_test = np.delete(SOH_test, [245])
    ...
    if mode == "SOH":
        ...
        EOL_mape = MAPE(np.array(SOH_test[0]), np.array(SOH_test_pred[0]))

        rmse_list.append(soh_rmse)
        mape_list.append(soh_mape)
        data_length = len(np.array(SOH_test))
...
plt.text(0.6, 20, "[RMSE_AVG]\n   " + str(round(rmse_avg, 3)) + "%")
plt.text(1.6, 20, "[MAPE_AVG]\n   " + str(round(mape_avg, 3)) + "%")
