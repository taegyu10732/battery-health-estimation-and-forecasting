...
#
# Method group **G15**: Operating-profile cycle-gradient model. Architecture: CNN-Transformer encoder. Method tags: operating profile|cycle gradient|adaptive loss.
#
...
# %%
plt.plot(bat_dict["b1c7"]["summary"]["QD"])
...
def data_maker(inter_list, window_size):
    ...
    return train_x, train_x_OP, train_x_Cycle, train_y


# %%
train_x, train_x_OP, train_x_Cycle, train_y = data_maker(train_bat_key, 50)

# %%
val_x, val_x_OP, val_x_Cycle, val_y = data_maker(val_bat_key, 50)
...
# lam_3 = torch.tensor([30.0]).to(DEVICE)
...
for epoch in range(0, 200):
    idx = 0
    lam_1_list.append(float(lam_1))
    lam_2_list.append(float(lam_2))
    ...
    for (
        ...
        RUL_N = gradient_calculator(SOH_pred, batch_X3).squeeze(1)

        l1 = loss_fn(SOH_pred, batch_y1)

        physics_value = RUL_N + 1

        empty_tensor = torch.zeros(len(physics_value)).to(DEVICE)

        l2 = loss_fn_physics(physics_value, empty_tensor)

        # loss = lam_1*l1 + lam_2*l2 - torch.log(lam_1*lam_2)
        # loss = l1
        loss = l1 + 0.07 * l2

        train_loss.append(float(l1))
        physics_loss.append(float(l2))
        ...
        with torch.no_grad():
            model.eval()
            SOH_pred_val = model(val_x_tensor, val_x_OP_tensor, val_x_Cycle_tensor)
            SOH_pred_val = SOH_pred_val.cpu().detach().numpy()
            SOH_val_loss = mean_squared_error(val_y, SOH_pred_val) ** 0.5
            # SOH_val_loss = mape_loss(val_y_tensor, SOH_pred_val).cpu().detach().numpy()
            SOH_val_loss = float(SOH_val_loss)
        ...
        if SOH_val_loss < SOH_loss_best:
            SOH_loss_best = SOH_val_loss
            torch.save(model.state_dict(), artifact_path("RUL_prediction_OP_50_gradient_9"))
    ...
    physics_loss_list.append(round(float(physics_loss), 4))
...
# %%
plt.plot(train_loss_list[10:], color="blue", label="train loss")
plt.plot(physics_loss_list[10:], color="red", label="physics loss")
plt.plot(val_loss_list[10:], color="black", label="val_loss")
...
lam_1 = torch.tensor([-3.0]).to(DEVICE)
lam_2 = torch.tensor([0.0]).to(DEVICE)
# lam_3 = torch.tensor([30.0]).to(DEVICE)
...
optimizer = torch.optim.Adam(model.parameters(), lr=0.001)
optimizer_2 = torch.optim.Adam([lam_1] + [lam_2], lr=0.001)
loss_fn = torch.nn.MSELoss()
loss_fn_physics = torch.nn.MSELoss()
...
for epoch in range(0, 1000):
    ...
    for (
        ...
        l2 = torch.sqrt(loss_fn_physics(physics_value, empty_tensor))

        loss = torch.exp(-lam_1) * l1 + torch.exp(-lam_2) * l2 + lam_1 + lam_2
        # loss = l1 + 0.08*l2
        # loss = l1
        ...
        if SOH_val_loss < SOH_loss_best:
            SOH_loss_best = SOH_val_loss
            torch.save(model.state_dict(), artifact_path("RUL_prediction_OP_50_gradient_test_1"))
        # print(float(l1), float(l2), SOH_val_loss)
...
for k in range(0, 14):
    ...
    model_new.load_state_dict(torch.load(artifact_path("RUL_prediction_OP_50_gradient_8")))

    test_x, test_x_OP, test_x_Cycle, test_y = data_maker(battery_list, 50)
    ...
    with torch.no_grad():
        model_new.eval()
        SOH_test_pred = model_new(test_x_tensor, test_x_OP_tensor, test_x_Cycle_tensor)
    ...
    if mode == "SOH":
        ...
        plt.text(1, data_length * 0.10, "RMSE : " + str(round(soh_rmse, 2)), fontsize=13)
