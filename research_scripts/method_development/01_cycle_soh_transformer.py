...
#
# Method group **G32**: Pre-normalization cycle-SOH model. Architecture: CNN-Transformer encoder. Method tags: joint cycle and SOH.
#
...
test_bat_key = ["b1c11", "b2c19", "b1c12", "b2c18", "b2c10"]
...
for epoch in range(0, 300):
    ...
    for batch_X1, batch_X2, batch_y1, batch_y2 in data_loader:
        ...
        if SOH_val_loss < SOH_loss_best:
            ...
            torch.save(model.state_dict(), artifact_path("SOH_Cycle_estimation_1"))
...
for k in range(len(test_bat_key)):
    ...
    model_new = SOH(4, inter_value, 4, 2, 1, 9).to(DEVICE)
    model_new.load_state_dict(torch.load(artifact_path("SOH_Cycle_estimation")))
    model_new.eval()
    ...
    if mode == "Cycle":
        ...
        plt.text(1, 0.3, "RMSE : " + str(round(cycle_rmse, 5)), fontsize=20)
