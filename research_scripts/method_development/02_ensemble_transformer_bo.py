...
#
# Method group **G33**: Ensemble Transformer Bayesian optimization. Architecture: CNN-Transformer encoder. Method tags: ensemble|Optuna|observation horizon.
#
        ...
        if cycle_num != "1":
            for i in range(11, data_length):
                ...
                if Charge_SOC > 80.1:
                    ...
                    temporal_data.append(Current_data[::10])
                    temporal_data.append(Voltage_data[::10])
                    temporal_data.append(Temperature_data[::10])
            ...
            for i in range(11, len(bat_dict[battery_key]["cycles"][cycle_num]["Qd"])):
                ...
                if Discharge_SOC > 0.01:
                    ...
                    temporal_data.append(Discharge_Voltage[::10])

                    break
            y_true.append(bat_dict[battery_key]["summary"]["QD"][int(cycle_num)])
            whole_data.append(np.array(temporal_data))

for i in range(len(whole_data) - 1, 0, -1):
    if whole_data[i].shape != (4, 30):
        del whole_data[i]
        del y_true[i]
...
class SOH(nn.Module):
    def __init__(
        ...
        self.final_reggression1 = nn.Linear(20, 1)

        self.final_reggression2 = nn.Linear(100, 1)
        ...
        if cycle_num != "1":
            ...
            for i in range(11, len(bat_dict[battery_key]["cycles"][cycle_num]["Qd"])):
                ...
                if Discharge_SOC > 0.01:
                    ...
                    break
            y_val.append(bat_dict[battery_key]["summary"]["QD"][int(cycle_num)])
            whole_val_data.append(np.array(temporal_data))

for i in range(len(whole_val_data) - 1, 0, -1):
    if whole_val_data[i].shape != (4, 30):
...
for i in range(len(minus_val_data) - 1, 0, -1):
    if minus_val_data[i].shape != (4, 30):
...
def objective(trial):
    ...
    model = SOH(4, 30, first_head, first_layer, second_head, second_layer).to(DEVICE)
...
# %%
model = SOH(4, 30, 2, 3, 4, 10).to(DEVICE)
model = model.train()
optimizer = torch.optim.Adam(model.parameters())
loss_fn = torch.nn.MSELoss()
...
for epoch in range(0, 100):
    ...
    for batch_X1, batch_X2, batch_y in data_loader:
        ...
        print("epoch : ", epoch, "idx : ", idx, " train_loss :", loss, " val_loss :", val_loss)

        if val_loss < val_loss_best:
            val_loss_best = val_loss
            torch.save(model.state_dict(), artifact_path("ensemble_transformer_300_sam"))
        idx += 1
...
for k in range(len(test_bat_key) - 1):
    ...
    model_new = SOH(4, 30, 2, 3, 4, 10).to(DEVICE)
    model_new.load_state_dict(torch.load(artifact_path("ensemble_transformer_300_sam")))
    model_new.eval()
    ...
    for i in range(len(whole_test_data) - 1, 0, -1):
        if whole_test_data[i].shape != (4, 30):
