...
#
# Method group **G35**: Empirical physics fusion. Architecture: CNN-Transformer encoder. Method tags: empirical fusion|log-cycle.
#
...
# test_bat_key = ['b1c9','b1c12','b1c21','b1c29','b1c39','b1c45' ,'b2c18', 'b2c30', 'b2c42', 'b2c21']
...
def objective(trial):
    ...
    second_head = trial.suggest_categorical("second_head", [1, 2, 4, 5, 10])
...
lam_2 = torch.nn.Parameter(lam_2)
lam_3 = torch.nn.Parameter(lam_3)
lam_4 = torch.nn.Parameter(lam_3)
...
for epoch in range(0, 300):
    ...
    for batch_X1, batch_X2, batch_y1, batch_y2 in data_loader:
        ...
        loss = (
            ...
            + l3 * lam_3
            + l4 * lam_4
            - torch.log(lam_1 * lam_2 * lam_3 * lam_4)
        )
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        if Total_val_loss < Total_loss_best:
            ...
            torch.save(
                model.state_dict(),
                artifact_path("empirical_fused_cycle_estimation_with_correct_data"),
            )
...
for k in range(len(test_bat_key) - 1):
    ...
    model_new.load_state_dict(
        torch.load(artifact_path("empirical_fused_cycle_estimation_with_correct_data"))
    )
    model_new.eval()
