...
#
# Method group **G30**: Light parameter-given model. Architecture: CNN-LSTM. Method tags: given parameters|light model.
#
...
plt.scatter(
    np.array(train_parameter)[0, id_],
    len(bat_dict[train_bat_key[0]]["summary"]["QD"]),
    label="train",
    color="black",
)
for i in range(1, len(train_bat_key)):
    plt.scatter(
...
plt.scatter(
    np.array(val_parameter)[0, id_],
    len(bat_dict[val_bat_key[0]]["summary"]["QD"]),
    label="val",
    color="blue",
)
for i in range(1, len(val_bat_key)):
    plt.scatter(
...
plt.scatter(
    np.array(test_parameter)[0, id_],
    len(bat_dict[test_bat_key[0]]["summary"]["QD"]),
    label="test",
    color="red",
)
for i in range(1, len(test_bat_key)):
    plt.scatter(
...
plt.xlabel("Fitted Parameter b", fontsize=20)
...
for epoch in range(0, 2000):
    ...
    for batch_X1, batch_y1, batch_y2, batch_y3 in data_loader:
        ...
        par_c = batch_y3[:, 2:3]
        par_d = batch_y3[:, 3:4]
        physics_term = par_a * torch.exp(Cycle_integrated * par_b) + par_c * torch.exp(
            Cycle_integrated * par_d
        )
        ...
        par_a_val = val_parameter_tensor[:, 0:1]
        par_b_val = val_parameter_tensor[:, 1:2]
        par_c_val = val_parameter_tensor[:, 2:3]
        par_d_val = val_parameter_tensor[:, 3:4]
        ...
        physics_term_val = par_a_val * torch.exp(
            Cycle_integrated_val * par_b_val
        ) + par_c_val * torch.exp(Cycle_integrated_val * par_d_val)
        ...
        if RUL_val_loss < loss_best:
            ...
            torch.save(
                model.state_dict(), artifact_path("physics_prediction_parameter_given_light_5")
            )
            print(
                "epoch",
                epoch,
...
model_new.load_state_dict(torch.load(artifact_path("physics_prediction_parameter_given_light_1")))
model_new.eval()
...
# fitted_function = test_param_pred[index][0] * np.exp(test_param_pred[index][1] * X/1400) + test_param_pred[index][2] * np.exp(test_param_pred[index][3] * X/1400)
# plt.plot(X, fitted_function, label='fitted equation')
