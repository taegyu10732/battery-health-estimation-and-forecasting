...
#
# Method group **G28**: Additional-information physics model. Architecture: CNN-LSTM. Method tags: additional information|100th cycle|NTK.
#
...
for battery_key in train_bat_key:
    ...
    cycle_list = []
    cap_list = [
        bat_dict[battery_key]["summary"]["QD"][0],
        bat_dict[battery_key]["summary"]["QD"][100],
    ]
...
class DenseNet(nn.Module):
    def __init__(self):
        ...
        self.par_network = nn.Sequential(
            nn.Linear(8, 30),
            nn.LeakyReLU(),
            nn.Linear(30, 30),
            nn.LeakyReLU(),
    ...
    def forward(self, x):
        ...
        x = x.squeeze(2)
        EOL = x[:, -8:]
        par = self.par_network(EOL)

        return EOL.squeeze(1), par.squeeze(1)
...
lam_2 = torch.tensor([0.1]).to(DEVICE)
...
# optimizer_1 = torch.optim.Adam([lam_1], lr=0.001)
# optimizer_2 = torch.optim.Adam([lam_2], lr=0.001)
...
for epoch in range(0, 2000):
    ...
    for batch_X1, batch_y1, batch_y2 in data_loader:
        ...
        # cycle_loss = torch.sqrt(loss_fn(Cycle_pred, batch_y1))
        # cycle_loss = loss_fn(Cycle_pred, batch_y1)
        Cycle_integrated = torch.cat(
            (
                (torch.ones(Cycle_pred.shape[0], 1) / 1400).to(DEVICE),
                (torch.ones(Cycle_pred.shape[0], 1) * 100 / 1400).to(DEVICE),
                Cycle_pred,
            ),
            dim=1,
        )

        par_a = -torch.abs(Param_pred[:, 0:1]) / 100
        par_b = torch.abs(Param_pred[:, 1:2])
        ...
        # physics_loss =loss_fn(physics_term, batch_y2)
        ...
        # loss = lam_1*cycle_loss + lam_2 *physics_loss
        # loss = cycle_loss
        # loss = cycle_loss + 2*physics_loss
        # loss = physics_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        Cycle_pred_val, Param_pred_val = model(whole_val_data_tensor)
        Cycle_integrated_val = torch.cat(
            (
                (torch.ones(Cycle_pred_val.shape[0], 1) / 1400).to(DEVICE),
                (torch.ones(Cycle_pred_val.shape[0], 1) * 100 / 1400).to(DEVICE),
                Cycle_pred_val,
            ),
            dim=1,
        )

        par_a_val = -torch.abs(Param_pred_val[:, 0:1]) / 100
        par_b_val = torch.abs(Param_pred_val[:, 1:2])
        ...
        # physics_term_val = par_a_val*torch.exp(Cycle_integrated_val * par_b_val) + par_c_val*torch.exp(Cycle_integrated_val * par_d_val)
        ...
        if RUL_val_loss < loss_best:
            ...
            torch.save(model.state_dict(), artifact_path("physics_prediction_100th_add_NTK"))
            print(
                "epoch",
                epoch,
                "/val_loss :",
                ...
                "/val_loss_P :",
                round(float(physics_loss_val), 4),
                "/Cycle_loss :",
...
model_new.load_state_dict(torch.load(artifact_path("physics_prediction_100th_add")))
model_new.eval()

RUL_train_pred, train_param_pred = model_new(whole_data_tensor)
...
RUL_val_pred, val_param_pred = model_new(whole_val_data_tensor)
RUL_val_pred = RUL_val_pred.cpu().detach().numpy()
RUL_val_pred = RUL_val_pred * 1400
val_param_pred = val_param_pred.cpu().detach().numpy()


RUL_test_pred, test_param_pred = model_new(whole_test_data_tensor)
...
True_x = np.append(100 / 1400, RUL_true[index])
True_x = np.append(0, True_x) * 1400
Pred_x = np.append(100, RUL_train_pred[index])
Pred_x = np.append(0, Pred_x)

plt.plot(Pred_x, Cap_true[index])
plt.scatter(Pred_x, Cap_true[index], label="Predict")
plt.plot(True_x, Cap_true[index])
plt.scatter(True_x, Cap_true[index], label="True")
...
fitted_function = (
    -np.abs(train_param_pred[index][0])
    / 100
    * np.exp(np.abs(train_param_pred[index][1]) * X / 1400)
    + train_param_pred[index][2]
)
...
True_x = np.append(100 / 1400, RUL_val[index])
True_x = np.append(0, True_x) * 1400
Pred_x = np.append(100, RUL_val_pred[index])
Pred_x = np.append(0, Pred_x)


plt.plot(Pred_x, Cap_val[index])
plt.scatter(Pred_x, Cap_val[index], label="Prediction")
plt.plot(True_x, Cap_val[index])
plt.scatter(True_x, Cap_val[index], label="True")
...
fitted_function = (
    -np.abs(val_param_pred[index][0]) / 100 * np.exp(np.abs(val_param_pred[index][1]) * X / 1400)
    + val_param_pred[index][2]
)
...
True_x = np.append(100 / 1400, RUL_test[index])
True_x = np.append(0, True_x) * 1400
Pred_x = np.append(100, RUL_test_pred[index])
Pred_x = np.append(0, Pred_x)

plt.plot(Pred_x, Cap_test[index], color="red")
plt.scatter(Pred_x, Cap_test[index], label="Predict", color="red")
plt.plot(True_x, Cap_test[index], color="blue")
plt.scatter(True_x, Cap_test[index], label="True", color="blue")
data_length = len(bat_dict[test_bat_key[index]]["summary"]["QD"])
X = np.linspace(1, data_length, data_length, dtype=int)
# X1 = np.linspace(1, RUL_test_pred[index][-1], 2000,dtype=int)
plt.plot(X, bat_dict[test_bat_key[index]]["summary"]["QD"], label="Real Capcity", color="black")

# fitted_function = test_param_pred[index][0] * np.exp(test_param_pred[index][1] * X/1400) + test_param_pred[index][2]

# fitted_function = -np.abs(test_param_pred[index][0])/100 * np.exp(np.abs(test_param_pred[index][1])*2 * X/1400) + test_param_pred[index][2]*np.exp(test_param_pred[index][3] * X/1400)
fitted_function = (
    -np.abs(test_param_pred[index][0]) / 100 * np.exp(np.abs(test_param_pred[index][1]) * X / 1400)
    + test_param_pred[index][2]
)
# fitted_function = test_param_pred[index][0]* np.exp(test_param_pred[index][1] * X/1400) + test_param_pred[index][2]
plt.plot(X, fitted_function, label="fitted equation", color="green")

print(test_param_pred[index][0], test_param_pred[index][1], test_param_pred[index][2])
plt.ylabel("Capcity", fontsize=15)
plt.xlabel("Cycle", fontsize=15)
...
# %%
for index in range(0, len(RUL_test)):
    Pred_x = np.append(100, RUL_test_pred[index])
    Pred_x = np.append(0, Pred_x)
    plt.plot(Pred_x, Cap_test[index])
