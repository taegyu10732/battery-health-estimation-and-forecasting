...
#
# Method group **G29**: Constrained-parameter physics model. Architecture: CNN-LSTM. Method tags: parameter constraints|physics loss.
#
...
for index in range(0, 70):
    plt.plot(bat_dict[train_bat_key[index]]["summary"]["QD"])
plt.ylim([0.88, 1.12])
plt.ylabel("Cap(Ah)", fontsize=15)
...
class DenseNet(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(1, 32, 4, batch_first=True)
...
for epoch in range(0, 2000):
    ...
    for batch_X1, batch_y1, batch_y2, batch_y3 in data_loader:
        ...
        # par_b = torch.exp(Param_pred[:, 1:2])
        ...
        # loss = cycle_loss
        # loss = 1*cycle_loss + 4.0*physics_loss
        # loss = physics_loss
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        # par_b_val = torch.exp(Param_pred_val[:, 1:2])
        ...
        if RUL_val_loss < loss_best:
            ...
            torch.save(
                model.state_dict(), artifact_path("physics_prediction_parameter_modified_gradient")
            )
            print(
                "epoch",
                epoch,
    ...
    lam_1_list.append((lam_1 * cycle_loss).cpu().detach().numpy())
    lam_2_list.append((lam_2 * physics_loss).cpu().detach().numpy())

# %%
plt.plot(lam_1_list[300:], label="lam1")
plt.plot(lam_2_list[300:], label="lam2")
...
model_new.load_state_dict(
    torch.load(artifact_path("physics_prediction_parameter_modified_gradient"))
)
model_new.eval()
...
plt.plot(np.append(index, RUL_test_pred[index]), Cap_test[index], color="red")
plt.scatter(np.append(0, RUL_test_pred[index]), Cap_test[index], label="Predict", color="red")
plt.plot(np.append(0, RUL_test[index] * 1400), Cap_test[index], color="blue")
plt.scatter(np.append(0, RUL_test[index] * 1400), Cap_test[index], label="True", color="blue")
